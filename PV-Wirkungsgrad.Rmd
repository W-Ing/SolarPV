...
DIREKT EINE VERSION NEU 

```{r}

meine_funktion <- function(verb.data, period){
...
# V_E_daten.melt <- melt(V_E_daten , id = "week", measure = c("batt_entladung", "direktverbrauch","netzbezug"))  
# 
# # liefert drei Spalten week - variable - value
# # week ausgewählt aus den Spalten von V_E_daten
# # variable hier werden die drei gelieferten Namen verwendet
# # denen in value ihre Werte zugeordnet sind
# 
# ggplot(V_E_daten.melt, aes(week, value, fill = variable)) +    # für melt
...
#    geom_text(aes(label=round(value,-0)), size= 3, data = V_E_daten.melt, position = position_stack(vjust=0.5))

V_E_daten.gather <<- V_E_daten %>% 
     gather(key = "key", value = Wert,  netzbezug, batt_entladung, direktverbrauch  ) %>% 
     select(-leistung.pv)

ggplot(V_E_daten.gather, aes_(qperiod, V_E_daten.gather$Wert, fill = V_E_daten.gather$key)) +      # fuer gather
   geom_bar(stat = "identity", position = "stack") +
   labs(     x  = "Kalenderwoche",
             y  = "Von PV-Anlage in der Woche erzeugt (kWh)",
             fill  = " Art",
             title = "GATHER",
             subtitle = "differenziert nach Verwendung") +
   geom_text(aes(label=round(Wert,-0)), size= 3, data = V_E_daten.gather, position = position_stack(vjust=0.5))
}
meine_funktion(verbrauch, week)
meine_funktion(verbrauch, month)
...
myfile <- plotte_erzeugung_u_verbrauch(verbrauch, week, "Erzeugung",  "Titel") 

myfile.long <- gather(myfile , key = type,value = val, direktverbrauch,batt_ladung,netzeinspeisung)

 ggplot(myfile, aes(week, value, fill = variable)) +     #
   geom_bar(stat = "identity", position = "stack") +
   labs(     x  = "Kalenderwoche",
             y  = "Von PV-Anlage in der Woche erzeugt (kWh)",
             fill  = " Art",
             title = "Test",
             subtitle = "differenziert nach Verwendung") +
   geom_text(aes(label=round(value,-0)), size= 3, data = myfile, position = position_stack(vjust=0.5))
...
ggplot(zusfg.long, aes(week, value, fill = variable)) +
   ...
   labs(     x  = "Kalenderwoche",
             y  = "Von PV-Anlage in der Woche erzeugt (kWh)",
          fill  = " Art",
          title = "Verbrauch woechentlich",
          subtitle = "differenziert nach Verwendung") +
   geom_text(aes(label=round(value,-0)), size= 3, data = zusfg.long, position = position_stack(vjust=0.5))
zusfg.long <- melt(zusfg , id = "week", measure = c("netzbezug","batt_entladung", "direktverbrauch"))
ggplot(zusfg.long, aes(week, value, fill = variable)) +
   geom_bar(stat = "identity", position = "stack") +
   labs(     x  = "Kalenderwoche",
...
### Eigenverbrauch - Datenübersicht (Eigenverbrauch fürs Finanzamt) VARIANTE

Vier Diagramme: Erzeugung und Verbrauch sowie Wochen/Monatsbilanz

```{r}

# zusammenfassung_von_vars <- list(list('week','netzbezug','batt_entladung', 'direktverbrauch'), 
#                                  list('month','netzbezug','batt_entladung', 'direktverbrauch')) 

stat_erzeugung_verbrauch <- function(period){
...
zusfg <- verbrauch %>% 
 ...
 zusfg.long <- melt(zusfg , id = text, measure = c("netzbezug","batt_entladung", "direktverbrauch"))
 ggplot(zusfg.long, aes(week, value, fill = variable)) +     # GEHT NIcht - wird nicht asls Spalte erkannt
   geom_bar(stat = "identity", position = "stack") +
            labs(     x  = "Kalenderwoche",
                      y  = "Von PV-Anlage in der Woche erzeugt (kWh)",
            fill  = " Art",
