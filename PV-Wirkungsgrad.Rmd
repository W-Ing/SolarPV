...
Der Ladezustand wird in % der Batteriekapazität angegeben. Laut Herstellerangabe beträgt diese 9,8 kWh, von denen 9,3 KWh verfügbsr sind.
...
e)  von W in Wh werden umgerechnet:
   ...
   Weil die Leistung jeweils in 5min-Intervallen erhoben wird, wird in dieser Zeit die Arbeit W = P 5min = P (1/12) h verrichtet.
    Anm.: In der späteren Auswertung wird dies so interpretiert: Eine zur Zeit t erbrachte Leistung P führt zu einer el. Arbeit von $P\cdot 5 \mbox{min}$ im Zeitintervall $t \pm 2,5 \mbox{min}$
...
Die Rohdaten werden getrennt in "data" mit Variablen:
zeit batt_ladung batt_entladung ladezustand month week day hour ladediff ct

und "verbrauch" mit Variablen:
zeit leistung.pv netzeinspeisung netzbezug batt_ladung batt_entladung ladezustand month week day hour ladediff
