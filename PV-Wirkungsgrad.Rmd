...
temp <- monotonie_ladezustand(data)

temp    <- temp %>% select(-one_of(c("is_min","is_max","day_bat_in","day_bat_out","day_period_ladehub","daypd","len_daypd")))

temp <- temp %>%
   ungroup() %>% 
   mutate(  up  = ifelse(mono ==  1, 1, 0),
           down = ifelse(mono == -1, 1, 0),
            su  = ifelse(lag(up)   != up,   1, 0),
            sd  = ifelse(lag(down) != down, 1, 0),
            s   = su | sd) 
...
work <- temp %>% 
      mutate( nr   = cumsum(s)) %>% 
      select(-one_of(c("su", "sd", "s")))    %>% 
      group_by(nr) %>% 
           mutate(input    = sum(batt_ladung),
                  output   = sum(batt_entladung),
                  netto_output = output - input,
                  entladung= max(ladezustand)-min(ladezustand),
                  eins     = 1,
                  dauer = sum(eins) /12  # Dauer in Stunden, vorher 1 entspricht 5 min
                  ) %>% 
      slice(1)
...
work %>% 
   ggplot(aes(x = entladung, y = netto_output)) + 
   geom_point(aes(x = entladung, y = netto_output, color=dauer)) +
   geom_smooth(mapping = aes(x = entladung, y = netto_output ),method=lm, se =FALSE) #+
   #labs(
    #  x = "Entladung",
    #  y = "netto_output",
    #  color = "ccc" )
