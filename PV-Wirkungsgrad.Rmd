...
eta_1 <- 0.91   # versuchesweise angenommen
eta_2 <- 0.77
lade_data <- data %>% 
   select(zeit, month, week, day, hour, ct, batt_ladung, batt_entladung, ladezustand ) %>% 
   mutate(korr_ladung = batt_ladung) 

lade_data$korr_ladung[1]<- lade_data$korr_ladung[1]+5800  

temp_data <- lade_data %>%
     mutate(kombi_ent_ladung = eta_1*batt_ladung - batt_entladung/eta_2) %>% 
...
val
# lade_data <- lade_data %>%
#    mutate(cum_ladung = cumsum(eta_1*temp_ladung - batt_entladung/eta_2),
#           abweichung = abs(cum_ladung - ladezustand),
#           cum_fehler = max(abweichung)
#           )
...
# -------------------------------------------------------------------
# Berechnet maximale Abweichung von Ladezustand und der mit eta1/eta2 gewichteten kumulativen Ladung
# fehler_func <- function( df, e1, e2){
#    df <- df %>% 
#    mutate(cum_ladung = cumsum(e1*temp_ladung - batt_entladung/e2),
#           abweichung = abs(cum_ladung - ladezustand)) %>%
#    summarize(f = max(abweichung))
#    return(df$f[1]) 
# }
fehler_func <- function(e1, e2){
   temp_data <- lade_data %>%
     mutate(kombi_ent_ladung = e1*batt_ladung - batt_entladung/e2) %>% 
     mutate(cum_ladung = cumsum(kombi_ent_ladung)) %>%
     mutate(abweichung = abs(cum_ladung - ladezustand)) %>%
     summarize(f = max(abweichung))
...
source("versuch.R") # Erzeuge MAtrix der Paare von eta1 und eta2

Mat_EtaPaare <- Mat_EtaPaare %>% 
     mutate(fehler = 0  )

# Mat_EtaPaare <- Mat_EtaPaare %>% 
#      mutate(Fehlerwert = fehler_func(Mat_EtaPaare$v1, Mat_EtaPaare$v2))

laenge_eta_paare <- nrow(Mat_EtaPaare)

for (i in 1:laenge_eta_paare){
      Mat_EtaPaare$fehler[i] = fehler_func(Mat_EtaPaare$v1[i], Mat_EtaPaare$v2[i])     
}
class(Mat_EtaPaare)
